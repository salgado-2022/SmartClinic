...
<div class="card shadow-sm">
    <div class="card-body">
        <form action="/patients/<?= (int)$patient['id'] ?>/update" method="POST" novalidate>
            <div class="mb-3">
                <label for="full_name" class="form-label">Nombre Completo</label>
                <input type="text" class="form-control <?= !empty($errors['full_name']) ? 'is-invalid' : '' ?>"
                       id="full_name" name="full_name"
                       value="<?= htmlspecialchars($data['full_name'] ?? $patient['full_name']) ?>" required>
            ...
            <div class="mb-3">
                ...
                <input type="date" class="form-control <?= !empty($errors['date_of_birth']) ? 'is-invalid' : '' ?>"
                       ...
                       value="<?= htmlspecialchars($data['date_of_birth'] ?? $patient['date_of_birth']) ?>" required>
            ...
            <div class="mb-3">
                ...
                <input type="email" class="form-control <?= !empty($errors['email']) ? 'is-invalid' : '' ?>"
                       id="email" name="email"
                       value="<?= htmlspecialchars($data['email'] ?? $patient['email']) ?>" required>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control <?= !empty($errors['phone']) ? 'is-invalid' : '' ?>"
                       id="phone" name="phone"
                       value="<?= htmlspecialchars($data['phone'] ?? $patient['phone']) ?>" required>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control <?= !empty($errors['id_document']) ? 'is-invalid' : '' ?>"
                       ...
                       value="<?= htmlspecialchars($data['id_document'] ?? $patient['id_document']) ?>" required>
